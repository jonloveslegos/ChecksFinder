...
else if (ds_map_find_value(async_load, "type") == network_type_data)
{
	...
	for (var i = 0; i < array_length(_parsed_message); i++)
	{
		if (_parsed_message[i].cmd == "Connected")
		{
			//show_message(_parsed_message[i].missing_locations)
			global.missing_locations = []
			global.missing_locations = _parsed_message[i].missing_locations
			global.checksgotten = 25-array_length(_parsed_message[i].missing_locations)
			room_goto_next()
		}
		else if (_parsed_message[i].cmd == "ReceivedItems")
		{
			//show_message(_parsed_message[i].items)
			for (var e = 0; e < array_length(_parsed_message[i].items); e++)
			{
		        if (_parsed_message[i].items[e].item == 80000)
		            scr_get_item("width")
		        if (_parsed_message[i].items[e].item == 80001)
		            scr_get_item("height")
